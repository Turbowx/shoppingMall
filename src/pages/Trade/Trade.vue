<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="item in addresses" :key="item.id">
        <span class="username" :class="{selected:item.isDefault == 1}">{{ item.consignee }}</span>
        <p @click="changeDefault(item,addresses)">
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="(orderDetail,index) in orderInfo.detailArrayList" :key="index">
          <li>
          ...
          <li>
            <h3>￥{{orderDetail.orderPrice}}.00</h3>
          </li>
    ...
    <div class="sub clearFix">
      ...
      <a class="subBtn" @click="toPayView">提交订单</a>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapState({
      addresses(state){
        return state.trade.addresses;
      },
      orderInfo(state){return state.trade.orderInfo}
    }),
    // ...mapGetters(["orderDetailList"]),
    //选中的地址
    userDefaultAddress(){
      return this.addresses.find(item=>item.isDefault == 1) || {};
    },
    //商品数目
    goodsNum(){
      let sum = 0;
      this.orderInfo.detailArrayList.forEach(item=>sum+=item.skuNum);
      return sum;
    }
  },
  methods: {
    //选择地址
    changeDefault(item,addresses){
    ...
    async toPayView(){
      ...
      let res = await this.$API.reqPlaceOrder(tradeNo,data)
      console.log("tijao",res);
      if(res.code == 200){
